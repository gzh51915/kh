...
Router.post('/order', (req, res) => { //增
    db.create('order')
    const { order } = req.params;
    const result = db.getCollection('list').insert(order);
    if (result.length > 0) {
        res.send({
            code: 1,
            data: result,
...
Router.get('/:id', async (req, res) => {
    ...
    if (id === "all-e0") {
        ...
        move = {destinationSingleCity : "广州"}
    } else {
        move = { _id: id }
    }
    const result = await db.find('list', move, { limit, skip, sort });
    if (result.length > 0) {
        res.send({
            code: 1,
            data: result,
